...
in Community APIcast operator. For example: tag: v3.14.0. 
These are examples from previous Community releases:
```asciidoc
...
* Create PR from community release development branch to a **3scale-2.X-stable** branch (PR example: https://github.com/3scale/apicast-operator/pull/226)
  * Do development, Test and Merge PR to `3scale-2.X-stable` branch
    * **Notes**:
      * **Building the PR will create a Community release image** in the following repository (example for release v0.7.1) https://quay.io/repository/3scale/apicast-operator?tab=tags&tag=v0.7.1
      * _After merging the PR, and Community Image creation and testing, - create another PR to revert Community release
       changes to the **3scale-2.X-stable** branch. This will allow us to continue using the stable branch for future minor/patch releases._
